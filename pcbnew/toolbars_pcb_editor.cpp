...
ACTION_TOOLBAR_CONTROL PCB_ACTION_TOOLBAR_CONTROLS::viaDiameter( "control.PCBViaDia", _( "Via diameter selector" ),
                                                                 ...
                                                                 { FRAME_PCB_EDITOR } );
ACTION_TOOLBAR_CONTROL PCB_ACTION_TOOLBAR_CONTROLS::currentVariant( "control.PCBCurrentVariant", _( "Current variant" ),
...
void PCB_EDIT_FRAME::configureToolbars()
{
    ...
    auto trackWidthSelectorFactory = [this]( ACTION_TOOLBAR* aToolbar )
    {
        ...
        {
            m_SelTrackWidthBox = new wxChoice( aToolbar, ID_AUX_TOOLBAR_PCB_TRACK_WIDTH, wxDefaultPosition,
    ...
    auto viaDiaSelectorFactory = [this]( ACTION_TOOLBAR* aToolbar )
    {
        ...
        {
            m_SelViaSizeBox =
                    new wxChoice( aToolbar, ID_AUX_TOOLBAR_PCB_VIA_SIZE, wxDefaultPosition, wxDefaultSize, 0, nullptr );
        }
    ...
    auto variantSelectionCtrlFactory = [this]( ACTION_TOOLBAR* aToolbar )
    {
        ...
        {
            m_CurrentVariantCtrl = new wxChoice( aToolbar, ID_AUX_TOOLBAR_PCB_VARIANT_SELECT, wxDefaultPosition,
                                                 wxDefaultSize, 0, nullptr );
        }
...
static wxString ComboBoxUnits( EDA_UNITS aUnits, double aValue, bool aIncludeLabel = true )
{
    ...
    switch( aUnits )
    {
    default: wxASSERT_MSG( false, wxT( "Invalid unit" ) ); KI_FALLTHROUGH;
...
void PCB_EDIT_FRAME::UpdateTrackWidthSelectBox( wxChoice* aTrackWidthSelectBox, bool aShowNetclass, bool aShowEdit )
{
...
void PCB_EDIT_FRAME::UpdateViaSizeSelectBox( wxChoice* aViaSizeSelectBox, bool aShowNetclass, bool aShowEdit )
{
    ...
    for( unsigned ii = 1; ii < GetDesignSettings().m_ViasDimensionsList.size(); ii++ )
    {
        ...
        {
            priStr =
                    ComboBoxUnits( primaryUnit, diam, false ) + wxT( " / " ) + ComboBoxUnits( primaryUnit, hole, true );
...
void PCB_EDIT_FRAME::ToggleSearch()
{
    ...
    if( m_ShowSearch )
    {
        ...
        {
            SetAuiPaneSize( m_auimgr, searchPaneInfo, settings->m_AuiPanels.search_panel_width, -1 );
        }
...
void PCB_EDIT_FRAME::ToggleAssistantPanel()
{
    PCBNEW_SETTINGS* settings = GetPcbNewSettings();
    wxAuiPaneInfo&   assistantPaneInfo = m_auimgr.GetPane( AssistantPaneName() );

    m_ShowAssistantPanel = assistantPaneInfo.IsShown();
    m_ShowAssistantPanel = !m_ShowAssistantPanel;

    if( !m_ShowAssistantPanel && m_assistantPanel )
    {
        m_AssistantPanelWidth = std::max( m_assistantPanel->GetSize().x, ASSISTANT_PANE_MIN_WIDTH );
    }
    else if( m_ShowAssistantPanel )
    {
        SetAuiPaneSize( m_auimgr, assistantPaneInfo,
                        std::max( settings->m_AuiPanels.assistant_panel_width, ASSISTANT_PANE_MIN_WIDTH ), -1 );
    }

    assistantPaneInfo.Show( m_ShowAssistantPanel );
