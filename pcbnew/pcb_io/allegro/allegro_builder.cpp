...
{
    if( aLayerInfo.m_Class == LAYER_INFO::CLASS::CONSTRAINTS_REGION || aLayerInfo.m_Class == LAYER_INFO::CLASS::BOUNDARY
...
public:
    ...
    void FinalizeLayers()
    {
        ...
        for( size_t li = 0; li < numCuLayers; ++li )
        {
            ...
            PCB_LAYER_ID lId = ( it != resolvedMapping.end() ) ? it->second : getNthCopperLayer( li, numCuLayers );
    ...
    PCB_LAYER_ID GetLayer( const LAYER_INFO& aLayerInfo )
    {
        ...
        if( m_ClassCustomLayerLists.count( aLayerInfo.m_Class ) )
        {
            ...
            if( etchIt != m_ClassCustomLayerLists.end() && cLayerList == etchIt->second
     ...
     */
    bool IsLayerMapped( PCB_LAYER_ID aLayerId ) const { return aLayerId != m_unmappedLayer; }
    ...
    LSET GetRuleAreaLayers( const LAYER_INFO& aLayerInfo )
    {
        ...
        case LAYER_INFO::CLASS::PACKAGE_KEEPOUT:
        {
            ...
            {
            case LAYER_INFO::SUBCLASS::KEEPOUT_ALL: layerSet = LSET::AllCuMask(); break;
            case LAYER_INFO::SUBCLASS::KEEPOUT_TOP: layerSet = LSET{ F_Cu }; break;
            case LAYER_INFO::SUBCLASS::KEEPOUT_BOTTOM: layerSet = LSET{ B_Cu }; break;
            default: layerSet = LSET{ GetLayer( aLayerInfo ) }; break;
            }
            break;
        }
...
private:
    ...
    PCB_LAYER_ID addUserLayer( const wxString& aName )
    {
        ...
        m_board.GetDesignSettings().SetUserDefinedLayerCount( m_board.GetDesignSettings().GetUserDefinedLayerCount()
                                                              + 1 );
...
public:
    ...
    void QueuePolygonForZone( ZONE& aZone, SHAPE_POLY_SET aFilledArea, PCB_LAYER_ID aLayer )
    {
        ...
        m_FillInfos.emplace_back( FILL_INFO{ &aZone, aLayer, std::move( aFilledArea ) } );
    }
...
void BOARD_BUILDER::applyConstraintSets()
{
    ...
    for( const BLK_0x1D_CONSTRAINT_SET& csBlock : csWalker )
    {
        ...
        wxLogTrace( traceAllegroBuilder, "Constraint set '%s': line_width=%d nm, clearance=%d nm, dp_gap=%d nm",
...
void BOARD_BUILDER::applyNetConstraints()
{
    ...
    for( const auto& [widthNm, netKeys] : widthToNetKeys )
    {
        ...
        wxLogTrace( traceAllegroBuilder, "Created netclass '%s' (track width %d nm) with %zu nets", ncName, widthNm,
                    netKeys.size() );
    }
...
void BOARD_BUILDER::applyMatchGroups()
{
    ...
    for( const auto& [groupName, netKeys] : groupToNetKeys )
    {
        ...
        wxLogTrace( traceAllegroBuilder, "%s group '%s' -> netclass '%s' with %zu nets",
                    isDiffPair ? wxS( "Diff pair" ) : wxS( "Match" ), groupName, ncName, netKeys.size() );
    }

    wxLogTrace( traceAllegroBuilder, "Applied match groups: %d diff pairs, %d match groups (%zu total groups)", dpCount,
                mgCount, groupToNetKeys.size() );
}
                                                       ...
                                                       BOARD_ITEM_CONTAINER&       aParent )
{
    ...
    {
    case BLK_0x30_STR_WRAPPER::TEXT_ALIGNMENT::LEFT: text->SetHorizJustify( GR_TEXT_H_ALIGN_LEFT ); break;
    case BLK_0x30_STR_WRAPPER::TEXT_ALIGNMENT::CENTER: text->SetHorizJustify( GR_TEXT_H_ALIGN_CENTER ); break;
    case BLK_0x30_STR_WRAPPER::TEXT_ALIGNMENT::RIGHT: text->SetHorizJustify( GR_TEXT_H_ALIGN_RIGHT ); break;
    default: break;
    }
                                                                       ...
                                                                       int aNetcode )
{
    ...
    for( size_t i = 0; i < aPadstack.GetLayerCount(); ++i )
    {
        ...
        const ALLEGRO::PADSTACK_COMPONENT& padComp =
                aPadstack.m_Components[layerBaseIndex + BLK_0x1C_PADSTACK::LAYER_COMP_SLOT::PAD];
        const ALLEGRO::PADSTACK_COMPONENT& antiPadComp =
                aPadstack.m_Components[layerBaseIndex + BLK_0x1C_PADSTACK::LAYER_COMP_SLOT::ANTIPAD];
        const ALLEGRO::PADSTACK_COMPONENT& thermalComp =
                aPadstack.m_Components[layerBaseIndex + BLK_0x1C_PADSTACK::LAYER_COMP_SLOT::THERMAL_RELIEF];
        ...
        case PADSTACK_COMPONENT::TYPE_SHAPE_SYMBOL:
        {
            ...
            if( outline.PointCount() >= 3 )
            {
                ...
                int   anchorSize = static_cast<int>( std::min( bbox.GetWidth(), bbox.GetHeight() ) / 4 );
        ...
        case PADSTACK_COMPONENT::TYPE_PENTAGON:
        {
            ...
            BOX2I bbox = outline.BBox();
            int   anchorSize = static_cast<int>( std::min( bbox.GetWidth(), bbox.GetHeight() ) / 7 );
        ...
        if( antiPadComp.m_Type != PADSTACK_COMPONENT::TYPE_NULL )
        {
            ...
            {
                wxLogTrace( traceAllegroBuilder,
                            "Padstack %s: copper layer %zu antipad shape %d "
            ...
            {
                wxLogTrace( traceAllegroBuilder,
                            "Padstack %s: copper layer %zu unequal antipad "
                            "clearance X=%d Y=%d",
            ...
            {
                wxLogTrace( traceAllegroBuilder,
                            "Padstack %s: copper layer %zu antipad offset "
                            "%d, %d",
        ...
        if( thermalComp.m_Type != PADSTACK_COMPONENT::TYPE_NULL && !thermalGap.has_value() )
        {
            ...
            wxLogTrace( traceAllegroBuilder, "Padstack %s: thermal relief type=%d, gap=%snm", padStackName,
                        thermalComp.m_Type,
                        thermalGap.has_value() ? wxString::Format( "%d", thermalGap.value() ) : wxString( "N/A" ) );
        }
    ...
    for( size_t i = 0; i < aPadstack.m_NumFixedCompEntries; ++i )
    {
        ...
        wxLogTrace( traceAllegroBuilder, "Fixed padstack slot %zu: type=%d, W=%d, H=%d", i,
                    static_cast<int>( psComp.m_Type ), psComp.m_W, psComp.m_H );
    }
...
std::vector<std::unique_ptr<BOARD_ITEM>> BOARD_BUILDER::buildTrack( const BLK_0x05_TRACK& aTrackBlock, int aNetCode )
{
    ...
    for( const BLOCK_BASE* block : segWalker )
    {
        ...
        default: wxLogTrace( traceAllegroBuilder, "Unhandled segment type in track: %#04x", segType ); break;
        }
    }
    return items;
}
                                                ...
                                                ZONE_FILL_HANDLER&                    aZoneFillHandler )
{
    ...
    bool isCopperZone =
            ( layerInfo.m_Class == LAYER_INFO::CLASS::ETCH || layerInfo.m_Class == LAYER_INFO::CLASS::BOUNDARY );
...
void BOARD_BUILDER::applyZoneFills()
{
    ...
    wxLogTrace( traceAllegroBuilder, "Applying zone fill polygons from %zu collected fills", m_zoneFillShapes.size() );
    ...
    wxLogTrace( traceAllegroPerf,
                wxT( "    applyZoneFills unmatched loop: %.3f ms (%d shapes, %d teardrops)" ), //format:allow
