 ...
 */
std::vector<ZONE*> findCascadeZones( const ZonePriorityMap& aByPriority, unsigned aFromPriority, bool aCascadeUp,
                                     bool& aViable )
{
    ...
    for( auto it = aByPriority.find( p ); it != aByPriority.end(); it = aByPriority.find( p ) )
    {
...
protected:
    ACTION_MENU* create() const override { return new ZONE_PRIORITY_CONTEXT_MENU(); }
...
public:
    ...
    ACTION_MENU* create() const override { return new LOCK_CONTEXT_MENU( this->m_tool ); }
};
...
int BOARD_EDITOR_CONTROL::ImportSpecctraSession( const TOOL_EVENT& aEvent )
{
    ...
    fullFileName = wxFileSelector( _( "Specctra Session File" ), path, name,
                                   ...
                                   FILEEXT::SpecctraSessionFileWildcard(), wxFD_OPEN | wxFD_CHANGE_DIR, frame() );
...
int BOARD_EDITOR_CONTROL::ExportNetlist( const TOOL_EVENT& aEvent )
{
    ...
    if( !fn.IsDirWritable() )
    {
        DisplayErrorMessage( m_frame,
                             wxString::Format( _( "Insufficient permissions to folder '%s'." ), fn.GetPath() ) );
        return 0;
    }
    ...
    {
        COMPONENT* component = new COMPONENT( footprint->GetFPID(), footprint->GetReference(), footprint->GetValue(),
                                              footprint->GetPath(), { footprint->m_Uuid } );
...
int BOARD_EDITOR_CONTROL::ShowEeschema( const TOOL_EVENT& aEvent )
{
    ...
    wxFileName schematic( project.GetProjectPath(), project.GetProjectName(), FILEEXT::KiCadSchematicFileExtension );
...
int BOARD_EDITOR_CONTROL::ToggleAssistantPanel( const TOOL_EVENT& aEvent )
{
    getEditFrame<PCB_EDIT_FRAME>()->ToggleAssistantPanel();
    return 0;
}
...
int BOARD_EDITOR_CONTROL::TrackWidthInc( const TOOL_EVENT& aEvent )
{
    ...
    if( routerTool && routerTool->IsToolActive() && routerTool->Router()->Mode() == PNS::PNS_MODE_ROUTE_DIFF_PAIR )
    {
    ...
    else
    {
        ...
        if( routerTool && routerTool->IsToolActive()
            && routerTool->Router()->GetState() == PNS::ROUTER::RouterState::ROUTE_TRACK && bds.m_UseConnectedTrackWidth
            && !bds.m_TempOverrideTrackWidth )
        {
...
int BOARD_EDITOR_CONTROL::PlaceFootprint( const TOOL_EVENT& aEvent )
{
    ...
    while( TOOL_EVENT* evt = Wait() )
    {
        ...
        else if( fp && ( ZONE_FILLER_TOOL::IsZoneFillAction( evt ) || evt->IsAction( &ACTIONS::redo ) ) )
        {
            wxBell();
        }
        else
...
static bool mergeZones( EDA_DRAW_FRAME* aFrame, BOARD_COMMIT& aCommit, std::vector<ZONE*>& aOriginZones,
...
int BOARD_EDITOR_CONTROL::ZonePriorityMoveToTop( const TOOL_EVENT& aEvent )
{
    ...
    std::vector<ZONE*> cascadeDown = findCascadeZones( byPriority, maxOverlapping, false, cascadeDownViable );
    ...
    bool useDown = cascadeDownViable && ( !cascadeUpViable || cascadeDown.size() <= cascadeUp.size() );
...
int BOARD_EDITOR_CONTROL::ZonePriorityMoveToBottom( const TOOL_EVENT& aEvent )
{
    ...
    bool               cascadeUpViable = false;
    std::vector<ZONE*> cascadeUp = findCascadeZones( byPriority, minOverlapping, true, cascadeUpViable );
    ...
    if( canCascadeDown )
    {
        cascadeDown = findCascadeZones( byPriority, minOverlapping - 1, false, cascadeDownViable );
    }
    ...
    bool useUp = cascadeUpViable && ( !cascadeDownViable || cascadeUp.size() <= cascadeDown.size() );
...
int BOARD_EDITOR_CONTROL::AssignNetclass( const TOOL_EVENT& aEvent )
{
    ...
    DIALOG_ASSIGN_NETCLASS dlg( m_frame, netNames, board()->GetNetClassAssignmentCandidates(),
                                [this]( const std::vector<wxString>& aNetNames )
                                {
                                    ...
                                    {
                                        int curr_netCode =
                                                board()->GetNetInfo().GetNetItem( curr_netName )->GetNetCode();
...
void BOARD_EDITOR_CONTROL::DoSetDrillOrigin( KIGFX::VIEW* aView, PCB_BASE_FRAME* aFrame, EDA_ITEM* originViewItem,
...
void BOARD_EDITOR_CONTROL::setTransitions()
{
    ...
    Go( &BOARD_EDITOR_CONTROL::ToggleAssistantPanel, PCB_ACTIONS::showAssistantPanel.MakeEvent() );
