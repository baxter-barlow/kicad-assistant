...
EVT_UPDATE_UI_RANGE( ID_POPUP_PCB_SELECT_WIDTH1, ID_POPUP_PCB_SELECT_WIDTH8, PCB_EDIT_FRAME::OnUpdateSelectTrackWidth )
EVT_UPDATE_UI_RANGE( ID_POPUP_PCB_SELECT_VIASIZE1, ID_POPUP_PCB_SELECT_VIASIZE8, PCB_EDIT_FRAME::OnUpdateSelectViaSize )
// Drop files event
        ...
        m_eventCounterTimer( nullptr )
{
    ...
    m_ShowSearch = false;
    m_ShowAssistantPanel = false;
    m_ShowNetInspector = false;
    m_AssistantPanelWidth = ASSISTANT_PANE_DEFAULT_WIDTH;
    ...
    {
        m_exportNetlistAction = new TOOL_ACTION( "pcbnew.EditorControl.exportNetlist", AS_GLOBAL, 0, "",
                                                 _( "Netlist..." ), _( "Export netlist used to update schematics" ) );
    }

    // Create GAL canvas
    auto canvas = new PCB_DRAW_PANEL_GAL( this, -1, wxPoint( 0, 0 ), m_frameSize, GetGalDisplayOptions(),
...
#ifdef KICAD_IPC_API
    wxTheApp->Bind( EDA_EVT_PLUGIN_AVAILABILITY_CHANGED, &PCB_EDIT_FRAME::onPluginAvailabilityChanged, this );
#endif
    ...
    m_assistantPanel = new ASSISTANT_PANEL( this, *this );
    ...
    // Rows; layers 4 - 6
    m_auimgr.AddPane( m_tbTopMain, EDA_PANE().HToolbar().Name( wxS( "TopMainToolbar" ) ).Top().Layer( 6 ) );
    m_auimgr.AddPane( m_tbTopAux, EDA_PANE().HToolbar().Name( wxS( "TopAuxToolbar" ) ).Top().Layer( 5 ) );
    m_auimgr.AddPane( m_messagePanel, EDA_PANE().Messages().Name( wxS( "MsgPanel" ) ).Bottom().Layer( 6 ) );

    // Columns; layers 1 - 3
    m_auimgr.AddPane( m_tbLeft, EDA_PANE().VToolbar().Name( wxS( "LeftToolbar" ) ).Left().Layer( 3 ) );

    m_auimgr.AddPane( m_tbRight, EDA_PANE().VToolbar().Name( wxS( "RightToolbar" ) ).Right().Layer( 3 ) );

    m_auimgr.AddPane( m_appearancePanel,
                      EDA_PANE()
                              .Name( wxS( "LayersManager" ) )
                              .Right()
                              .Layer( 4 )
    ...
    m_auimgr.AddPane( m_selectionFilterPanel, EDA_PANE()
                                                      .Name( wxS( "SelectionFilter" ) )
                                                      .Right()
                                                      .Layer( 4 )
                                                      .Position( 2 )
    ...
    m_auimgr.AddPane( m_designBlocksPane, EDA_PANE()
                                                  .Name( DesignBlocksPaneName() )
                                                  .Right()
                                                  .Layer( 5 )
    ...
    m_auimgr.AddPane( m_propertiesPanel, EDA_PANE()
                                                 .Name( PropertiesPaneName() )
                                                 .Left()
                                                 .Layer( 5 )
    ...
    // Center
    m_auimgr.AddPane( GetCanvas(), EDA_PANE().Canvas().Name( wxS( "DrawFrame" ) ).Center() );
    ...
    m_auimgr.AddPane( m_assistantPanel, EDA_PANE()
                                                .Name( AssistantPaneName() )
                                                .Right()
                                                .Caption( _( "Assistant" ) )
                                                .PaneBorder( false )
                                                .MinSize( FromDIP( wxSize( ASSISTANT_PANE_MIN_WIDTH, 120 ) ) )
                                                .BestSize( FromDIP( wxSize( m_AssistantPanelWidth, 360 ) ) )
                                                .FloatingSize( FromDIP( wxSize( 520, 700 ) ) )
                                                .LeftDockable( false )
                                                .TopDockable( false )
                                                .BottomDockable( false )
                                                .DestroyOnClose( false )
    ...
    m_auimgr.GetPane( AssistantPaneName() ).Show( m_ShowAssistantPanel );
    ...
    if( aui_cfg.assistant_panel_width > 0 )
    {
        wxAuiPaneInfo& assistantPane = m_auimgr.GetPane( AssistantPaneName() );
        SetAuiPaneSize( m_auimgr, assistantPane, aui_cfg.assistant_panel_width, -1 );
    }
...

    {
        ...
        m_layerPairSettings->Bind( PCB_LAYER_PAIR_PRESETS_CHANGED,
                                   [&]( wxCommandEvent& aEvt )
                                   {
                                       ...
                                       std::span<const LAYER_PAIR_INFO> newPairInfos =
                                               m_layerPairSettings->GetLayerPairs();
...
void PCB_EDIT_FRAME::StartCrossProbeFlash( const std::vector<BOARD_ITEM*>& aItems )
{
    ...
    for( const BOARD_ITEM* item : aItems )
    {
        if( item->IsMoving() )
        {
            wxLogTrace( traceCrossProbeFlash, "StartCrossProbeFlash(PCB): aborted (items are moving)" );
            return;
        }
    }
    ...
    wxLogTrace( traceCrossProbeFlash, "StartCrossProbeFlash(PCB): timer start=%d id=%d", (int) started,
...
{
    wxLogTrace( traceCrossProbeFlash, "Timer(PCB) fired: phase=%d running=%d items=%zu", m_crossProbeFlashPhase,
    ...
    else
    {
        ...
        wxLogTrace( traceCrossProbeFlash, "Phase %d (PCB): restored %zu items", m_crossProbeFlashPhase,
...
void PCB_EDIT_FRAME::SetPageSettings( const PAGE_INFO& aPageSettings )
{
    ...
    DS_PROXY_VIEW_ITEM* drawingSheet =
            new DS_PROXY_VIEW_ITEM( pcbIUScale, &m_pcb->GetPageSettings(), m_pcb->GetProject(), &m_pcb->GetTitleBlock(),
                                    &m_pcb->GetProperties() );
...
void PCB_EDIT_FRAME::setupTools()
{
    ...
    m_toolManager->SetEnvironment( m_pcb, GetCanvas()->GetView(), GetCanvas()->GetViewControls(), config(), this );
...
void PCB_EDIT_FRAME::setupUIConditions()
{
    ...
    auto undoCond = [this]( const SELECTION& aSel )
    {
    ...
    auto groupWithDesignBlockLink = []( const SELECTION& aSel )
    {
        if( aSel.Size() != 1 )
            return false;
...
#define CHECK( x ) ACTION_CONDITIONS().Check( x )
    ...
    auto assistantPaneCond =
            [this] ( const SELECTION& )
            {
                return m_auimgr.GetPane( AssistantPaneName() ).IsShown();
            };
    ...
    mgr->SetConditions( PCB_ACTIONS::showAssistantPanel,   CHECK( assistantPaneCond ) );
...
void PCB_EDIT_FRAME::LoadSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    if( cfg )
    {
        ...
        m_ShowAssistantPanel = cfg->m_AuiPanels.show_assistant_panel;
        m_ShowNetInspector = cfg->m_AuiPanels.show_net_inspector;
        m_AssistantPanelWidth = std::max( cfg->m_AuiPanels.assistant_panel_width, ASSISTANT_PANE_MIN_WIDTH );
    }
}
...
void PCB_EDIT_FRAME::SaveSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    if( cfg )
    {
        ...
        if( m_assistantPanel )
        {
            wxAuiPaneInfo& assistantPaneInfo = m_auimgr.GetPane( AssistantPaneName() );
            m_ShowAssistantPanel = assistantPaneInfo.IsShown();
            cfg->m_AuiPanels.show_assistant_panel = m_ShowAssistantPanel;

            if( m_ShowAssistantPanel )
                m_AssistantPanelWidth = std::max( m_assistantPanel->GetSize().x, ASSISTANT_PANE_MIN_WIDTH );
        }

        cfg->m_AuiPanels.assistant_panel_width = std::max( m_AssistantPanelWidth, ASSISTANT_PANE_MIN_WIDTH );
    ...
    GetCanvas()->GetView()->UpdateAllItemsConditionally(
            [&]( KIGFX::VIEW_ITEM* aItem ) -> int
            {
                ...
                if( item->Type() == PCB_VIA_T )
                {
                    ...
                    // is changed
                    if( via->GetViaType() == VIATYPE::BLIND || via->GetViaType() == VIATYPE::BURIED
...
void PCB_EDIT_FRAME::ShowChangedLanguage()
{
    ...
    m_auimgr.GetPane( AssistantPaneName() ).Caption( _( "Assistant" ) );
    m_auimgr.Update();
...
ASSISTANT_PANEL_CONTEXT PCB_EDIT_FRAME::GetAssistantPanelContext() const
{
    ASSISTANT_PANEL_CONTEXT context;
    context.kind = ASSISTANT_CONTEXT_KIND::PCB;
    context.frameLabel = _( "PCB Editor" );

    wxFileName projectFn( Prj().GetProjectFullName() );

    if( projectFn.IsOk() )
    {
        context.projectName = projectFn.GetName();
        context.projectPath = projectFn.GetFullPath();
        context.workspacePath = projectFn.GetPath();
    }

    wxString boardPath;

    if( GetBoard() )
        boardPath = GetBoard()->GetFileName();

    if( !boardPath.IsEmpty() )
    {
        wxFileName boardFn = Prj().AbsolutePath( boardPath );
        boardPath = boardFn.GetFullPath();
    }

    context.hasProject = !context.projectPath.IsEmpty() || !boardPath.IsEmpty();
    context.documentLabel = _( "Board file" );
    context.documentPath = boardPath;
    return context;
}


void PCB_EDIT_FRAME::refreshAssistantPanel()
{
    if( m_assistantPanel )
        m_assistantPanel->RefreshContext();
}
...
void PCB_EDIT_FRAME::ShowFindDialog()
{
    if( !m_findDialog )
    {
        ...
        m_findDialog->SetCallback(
                std::bind( &PCB_SELECTION_TOOL::FindItem, m_toolManager->GetTool<PCB_SELECTION_TOOL>(), _1 ) );
    }
    ...
    if( selection.Size() == 1 )
    {
        ...
        switch( front->Type() )
        {
        case PCB_FOOTPRINT_T: findString = UnescapeString( static_cast<FOOTPRINT*>( front )->GetValue() ); break;
...
bool PCB_EDIT_FRAME::FetchNetlistFromSchematic( NETLIST& aNetlist, const wxString& aAnnotateMessage )
{
    ...
    catch( const IO_ERROR& e )
    {
        ...
        DisplayErrorMessage( this,
                             _( "Received an error while reading netlist.  Please "
...
int PCB_EDIT_FRAME::ShowExchangeFootprintsDialog( FOOTPRINT* aFootprint, bool aUpdateMode, bool aSelectedMode )
{
...
void PCB_EDIT_FRAME::ExchangeFootprint( FOOTPRINT* aExisting, FOOTPRINT* aNew, BOARD_COMMIT& aCommit,
                                        bool matchPadPositions, bool deleteExtraTexts, bool resetTextLayers,
                                        bool resetTextEffects, bool resetTextPositions, bool resetTextContent,
                                        bool resetFabricationAttrs, bool resetClearanceOverrides, bool reset3DModels,
                                        bool* aUpdated )
{
    ...
    processTextItem( aExisting->Value(), aNew->Value(), posShift, angleShift,
                     ...
                     aExisting->GetValue() == aExisting->GetFPID().GetLibItemName().wx_str(), resetTextLayers,
                     resetTextEffects, resetTextPositions, aUpdated );
...
DIALOG_BOOK_REPORTER* PCB_EDIT_FRAME::GetInspectDrcErrorDialog()
{
    if( !m_inspectDrcErrorDlg )
    {
        m_inspectDrcErrorDlg = new DIALOG_BOOK_REPORTER( this, INSPECT_DRC_ERROR_DIALOG_NAME, _( "Violation Report" ) );
    }
...
DIALOG_BOOK_REPORTER* PCB_EDIT_FRAME::GetInspectClearanceDialog()
{
    if( !m_inspectClearanceDlg )
    {
        m_inspectClearanceDlg =
                new DIALOG_BOOK_REPORTER( this, INSPECT_CLEARANCE_DIALOG_NAME, _( "Clearance Report" ) );
    }
...
DIALOG_BOOK_REPORTER* PCB_EDIT_FRAME::GetInspectConstraintsDialog()
{
    if( !m_inspectConstraintsDlg )
    {
        m_inspectConstraintsDlg =
                new DIALOG_BOOK_REPORTER( this, INSPECT_CONSTRAINTS_DIALOG_NAME, _( "Constraints Report" ) );
    }
...
DIALOG_BOOK_REPORTER* PCB_EDIT_FRAME::GetFootprintDiffDialog()
{
    ...
    {
        m_footprintDiffDlg =
                new DIALOG_BOOK_REPORTER( this, FOOTPRINT_DIFF_DIALOG_NAME, _( "Compare Footprint with Library" ) );
...
void PCB_EDIT_FRAME::OnEditItemRequest( BOARD_ITEM* aItem )
{
    switch( aItem->Type() )
    {
    case PCB_REFERENCE_IMAGE_T: ShowReferenceImagePropertiesDialog( aItem ); break;

    case PCB_BARCODE_T: ShowBarcodePropertiesDialog( static_cast<PCB_BARCODE*>( aItem ) ); break;

    case PCB_FIELD_T:
    case PCB_TEXT_T: ShowTextPropertiesDialog( static_cast<PCB_TEXT*>( aItem ) ); break;

    case PCB_TEXTBOX_T: ShowTextBoxPropertiesDialog( static_cast<PCB_TEXTBOX*>( aItem ) ); break;
    ...
    case PCB_PAD_T: ShowPadPropertiesDialog( static_cast<PAD*>( aItem ) ); break;

    case PCB_FOOTPRINT_T: ShowFootprintPropertiesDialog( static_cast<FOOTPRINT*>( aItem ) ); break;

    case PCB_TARGET_T: ShowTargetOptionsDialog( static_cast<PCB_TARGET*>( aItem ) ); break;
    ...
    case PCB_SHAPE_T: ShowGraphicItemPropertiesDialog( static_cast<PCB_SHAPE*>( aItem ) ); break;

    case PCB_ZONE_T: Edit_Zone_Params( static_cast<ZONE*>( aItem ) ); break;
    ...
    case PCB_GENERATOR_T: static_cast<PCB_GENERATOR*>( aItem )->ShowPropertiesDialog( this ); break;

    case PCB_MARKER_T: m_toolManager->GetTool<DRC_TOOL>()->CrossProbe( static_cast<PCB_MARKER*>( aItem ) ); break;

    case PCB_POINT_T: break;
