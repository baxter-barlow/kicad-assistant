...
static wxFileName ensureDefaultProjectTemplate()
{
    ...
    if( !infoFile.FileExists() )
    {
        ...
        info.Write( wxT( "<html><head><title>Default</title></head><body><h3>Default KiCad project "
                         "template.</h3></body></html>" ) );
        info.Close();
    }
    ...
    RunMainStack(
            [&]()
            {
                DIALOG_TEMPLATE_SELECTOR ps( m_frame, settings->m_TemplateWindowPos, settings->m_TemplateWindowSize,
                                             userTemplatesPath, systemTemplatesPath, settings->m_RecentTemplates );
...
int KICAD_MANAGER_CONTROL::ToggleAssistantPanel( const TOOL_EVENT& aEvent )
{
    m_frame->ToggleAssistantPanel();
    return 0;
}
...
void KICAD_MANAGER_CONTROL::setTransitions()
{
    ...
    Go( &KICAD_MANAGER_CONTROL::ToggleAssistantPanel, KICAD_MANAGER_ACTIONS::showAssistantPanel.MakeEvent() );
