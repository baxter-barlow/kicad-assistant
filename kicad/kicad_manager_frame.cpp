...
KICAD_MANAGER_FRAME::KICAD_MANAGER_FRAME( wxWindow* parent, const wxString& title, const wxPoint& pos,
                                          const wxSize& size ) :
        ...
        m_pcmUpdateCount( 0 )
{
    ...
    m_auimgr.AddPane( m_projectTreePane, EDA_PANE()
                                                 .Palette()
                                                 .Name( "ProjectTree" )
                                                 .Left()
                                                 .Layer( 1 )
                                                 .Caption( PROJECT_FILES_CAPTION )
                                                 .PaneBorder( false )
    ...
    m_auimgr.AddPane( m_historyPane, EDA_PANE()
                                             .Palette()
                                             .Name( "LocalHistory" )
                                             .Left()
                                             .Layer( 1 )
                                             .Position( 1 )
                                             .Caption( _( "Local History" ) )
                                             .PaneBorder( false )
                                             .Floatable( false )
                                             .Movable( false )
                                             .CloseButton( true )
    ...
    m_auimgr.AddPane( m_assistantPanel, EDA_PANE()
                                                ...
                                                .Caption( _( "Assistant" ) )
                                                .PaneBorder( false )
                                                .MinSize( m_assistantPanelWidth, FromDIP( 80 ) )
                                                .BestSize( m_assistantPanelWidth, FromDIP( 360 ) )
    ...
    if( m_showAssistantPanel )
        m_auimgr.GetPane( m_assistantPanel ).Show();

    wxSize client_size = GetClientSize();
    m_notebook = new wxAuiNotebook(
            this, wxID_ANY, wxPoint( client_size.x, client_size.y ), FromDIP( wxSize( 700, 590 ) ),
            wxAUI_NB_TOP | wxAUI_NB_CLOSE_ON_ALL_TABS | wxAUI_NB_TAB_MOVE | wxAUI_NB_SCROLL_BUTTONS | wxNO_BORDER );
    ...
    // Apply dark background colors to match the assistant panel color scheme
    if( KIPLATFORM::UI::IsDarkTheme() )
    {
        wxColour bg( 0x1b, 0x1d, 0x23 );
        wxColour surface( 0x22, 0x24, 0x30 );
        wxColour text( 0xd4, 0xd8, 0xe8 );
        wxColour textMuted( 0x8c, 0x92, 0xa8 );
        ...
        if( m_tbLeft )
        {
            m_tbLeft->SetBackgroundColour( surface );
        }

        // Project tree pane and tree control
        m_projectTreePane->SetBackgroundColour( surface );
        m_projectTreePane->SetForegroundColour( text );

        for( wxWindow* child : m_projectTreePane->GetChildren() )
        {
            child->SetBackgroundColour( surface );
            child->SetForegroundColour( text );
        }

        // Center notebook and launcher
        m_notebook->SetBackgroundColour( bg );
        m_launcher->SetBackgroundColour( bg );

        if( wxScrolledWindow* sw = dynamic_cast<wxScrolledWindow*>(
                    m_launcher->FindWindowByName( wxT( "m_scrolledWindow" ) ) ) )
        {
            sw->SetBackgroundColour( bg );
        }
        else
        {
            // Walk children to find the scrolled window
            for( wxWindow* child : m_launcher->GetChildren() )
            {
                if( dynamic_cast<wxScrolledWindow*>( child ) )
                {
        ...
        // Style launcher text labels
        for( wxWindow* child : m_launcher->GetChildren() )
        {
            if( wxScrolledWindow* sw = dynamic_cast<wxScrolledWindow*>( child ) )
            {
                for( wxWindow* gc : sw->GetChildren() )
                {
                    if( wxStaticText* st = dynamic_cast<wxStaticText*>( gc ) )
                        st->SetForegroundColour( text );
                }
            }
        }
        ...
        if( GetStatusBar() )
        {
            GetStatusBar()->SetBackgroundColour( surface );
            GetStatusBar()->SetForegroundColour( textMuted );
        }
    }
...
wxStatusBar* KICAD_MANAGER_FRAME::OnCreateStatusBar( int number, long style, wxWindowID id, const wxString& name )
{
            ...
            [this]( int aUpdateCount )
            {
                ...
                if( aUpdateCount > 0 )
                {
                    Pgm().GetNotificationsManager().CreateOrUpdate(
                            wxS( "pcm" ), _( "PCM Updates Available" ),
                            wxString::Format( _( "%d package update(s) available" ), aUpdateCount ), wxT( "" ) );
                }
                else
                {
...
#define ENABLE( x ) ACTION_CONDITIONS().Enable( x )
    ...
    auto historyCond = [this]( const SELECTION& )
    {
    ...
    auto assistantCond = [this]( const SELECTION& )
    {
        return AssistantPanelShown();
    };

    manager->SetConditions( KICAD_MANAGER_ACTIONS::showAssistantPanel, ACTION_CONDITIONS().Check( assistantCond ) );
...
ASSISTANT_PANEL_CONTEXT KICAD_MANAGER_FRAME::GetAssistantPanelContext() const
{
    ASSISTANT_PANEL_CONTEXT context;
    context.kind = ASSISTANT_CONTEXT_KIND::MANAGER;
    context.frameLabel = _( "Project Manager" );
    context.hasProject = Pgm().GetSettingsManager().IsProjectOpen();

    if( context.hasProject )
    {
        wxFileName projectFn( GetProjectFileName() );

        context.projectName = projectFn.GetName();
        context.projectPath = projectFn.GetFullPath();
        context.documentLabel = _( "Project file" );
        context.documentPath = projectFn.GetFullPath();
...
bool KICAD_MANAGER_FRAME::LoadProject( const wxFileName& aProjectFileName )
{
    ...
    if( lockFilePreExisted )
    {
        ...
        {
            wxString msg;
            msg.Printf( _( "Project '%s' is already open by '%s' at '%s'." ), fullPath, lockFile.GetUsername(),
...
void KICAD_MANAGER_FRAME::CreateNewProject( const wxFileName& aProjectFileName, bool aCreateStubFiles )
{
    ...
    if( !aProjectFileName.FileExists() )
    {
        ...
        else
        {
            ...
            if( !wxFileName::FileExists( srcFileName ) || !wxCopyFile( srcFileName, destFileName.GetFullPath() ) )
            {
...
void KICAD_MANAGER_FRAME::OnOpenFileInTextEditor( wxCommandEvent& event )
{
    ...
    wxFileDialog dlg( this, _( "Edit File in Text Editor" ), default_dir, wxEmptyString, wildcard, wxFD_OPEN );
...
void KICAD_MANAGER_FRAME::ShowChangedLanguage()
{
    ...
    m_auimgr.GetPane( m_assistantPanel ).Caption( _( "Assistant" ) );
    m_auimgr.Update();
...
void KICAD_MANAGER_FRAME::CommonSettingsChanged( int aFlags )
{
    ...
    if( m_lastToolbarIconSize == 0 || m_lastToolbarIconSize != settings->m_Appearance.toolbar_icon_size )
    {
...
void KICAD_MANAGER_FRAME::ProjectChanged()
{
    ...
    if( !Prj().IsNullProject() && Prj().GetProjectLock() == nullptr )
    {
...
void KICAD_MANAGER_FRAME::LoadSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    m_assistantPanelWidth = std::max( settings->m_AssistantPanelWidth, FromDIP( ASSISTANT_PANE_MIN_WIDTH ) );
    m_showHistoryPanel = settings->m_ShowHistoryPanel;
    m_showAssistantPanel = settings->m_ShowAssistantPanel;
}
...
void KICAD_MANAGER_FRAME::SaveSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    settings->m_ShowAssistantPanel = AssistantPanelShown();

    if( m_assistantPanel && settings->m_ShowAssistantPanel )
        m_assistantPanelWidth = std::max( m_assistantPanel->GetSize().x, FromDIP( ASSISTANT_PANE_MIN_WIDTH ) );

    settings->m_AssistantPanelWidth = std::max( m_assistantPanelWidth, FromDIP( ASSISTANT_PANE_MIN_WIDTH ) );
...
void KICAD_MANAGER_FRAME::OnIdle( wxIdleEvent& aEvent )
{
    ...
    if( Pgm().GetCommonSettings()->m_Session.remember_open_files )
    {
        int previousOpenCount =
                std::count_if( Prj().GetLocalSettings().m_files.begin(), Prj().GetLocalSettings().m_files.end(),
...
void KICAD_MANAGER_FRAME::ToggleAssistantPanel()
{
    wxAuiPaneInfo& pane = m_auimgr.GetPane( m_assistantPanel );
    bool           show = !pane.IsShown();

    if( !show && m_assistantPanel )
    {
        m_assistantPanelWidth = std::max( m_assistantPanel->GetSize().x, FromDIP( ASSISTANT_PANE_MIN_WIDTH ) );
    }
    else if( show )
    {
        pane.BestSize( FromDIP( wxSize( std::max( m_assistantPanelWidth, ASSISTANT_PANE_MIN_WIDTH ), 300 ) ) );
    }
...
bool KICAD_MANAGER_FRAME::AssistantPanelShown()
{
    return m_assistantPanel && m_auimgr.GetPane( m_assistantPanel ).IsShown();
}


void KICAD_MANAGER_FRAME::refreshAssistantPanel()
{
    if( m_assistantPanel )
