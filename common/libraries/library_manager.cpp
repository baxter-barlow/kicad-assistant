                                  ...
                                  std::vector<LIBRARY_TABLE_TYPE> aTablesToLoad )
{
    ...
    auto getTarget = [&]() -> std::map<LIBRARY_TABLE_TYPE, std::unique_ptr<LIBRARY_TABLE>>&
    {
        switch( aScope )
        {
        case LIBRARY_TABLE_SCOPE::GLOBAL: return m_tables;

        case LIBRARY_TABLE_SCOPE::PROJECT: return m_projectTables;

        default: wxCHECK_MSG( false, m_tables, "Invalid scope passed to loadTables" );
        }
    };
    ...
    for( LIBRARY_TABLE_TYPE type : aTablesToLoad )
    {
        ...
        if( fn.IsFileReadable() )
        {
            ...
            if( table->Type() != type )
            {
                ...
                wxLogWarning( wxS( "Library table '%s' has type %s but expected %s; skipping" ), fn.GetFullPath(),
...
void LIBRARY_MANAGER::loadNestedTables( LIBRARY_TABLE& aRootTable )
{
    ...
    std::function<void( LIBRARY_TABLE& )> processOneTable = [&]( LIBRARY_TABLE& aTable )
    {
...
{
public:
    explicit PCM_LIB_TRAVERSER( const wxString& aBasePath, LIBRARY_MANAGER& aManager, const wxString& aPrefix ) :
            m_manager( aManager ),
            ...
            m_lib_prefix( aPrefix )
    {
        ...
        m_designBlockTable =
                m_manager.Table( LIBRARY_TABLE_TYPE::DESIGN_BLOCK, LIBRARY_TABLE_SCOPE::GLOBAL ).value_or( nullptr );
    }
    ...
    wxDirTraverseResult OnDir( const wxString& dirPath ) override
    {
        ...
        else if( dirPath.EndsWith( designBlockExt ) && dir.GetDirCount() >= m_prefix_dir_count + 3
...
private:
    ...
    void addRowIfNecessary( LIBRARY_TABLE* aTable, const wxFileName& aSource, ADD_MODE aMode, int aExtensionLength )
    {
        wxString versionedPath =
                wxString::Format( wxS( "${%s}" ), ENV_VAR::GetVersionedEnvVarName( wxS( "3RD_PARTY" ) ) );
...
std::vector<LIBRARY_TABLE_TYPE> LIBRARY_MANAGER::InvalidGlobalTables()
{
    ...
    for( LIBRARY_TABLE_TYPE tableType :
         { LIBRARY_TABLE_TYPE::SYMBOL, LIBRARY_TABLE_TYPE::FOOTPRINT, LIBRARY_TABLE_TYPE::DESIGN_BLOCK } )
    {
...
void LIBRARY_MANAGER::LoadGlobalTables( std::initializer_list<LIBRARY_TABLE_TYPE> aTablesToLoad )
{
    ...
    auto cleanupRemovedPCMLibraries = [&]( LIBRARY_TABLE_TYPE aType )
    {
...
void LIBRARY_MANAGER::RegisterAdapter( LIBRARY_TABLE_TYPE aType, std::unique_ptr<LIBRARY_MANAGER_ADAPTER>&& aAdapter )
{
    std::scoped_lock lock( m_adaptersMutex );
...
std::optional<LIBRARY_TABLE*> LIBRARY_MANAGER::Table( LIBRARY_TABLE_TYPE aType, LIBRARY_TABLE_SCOPE aScope )
{
    switch( aScope )
    {
    case LIBRARY_TABLE_SCOPE::BOTH:
    case LIBRARY_TABLE_SCOPE::UNINITIALIZED: wxCHECK_MSG( false, std::nullopt, "Table() requires a single scope" );

    case LIBRARY_TABLE_SCOPE::GLOBAL:
    {
        if( !m_tables.contains( aType ) )
        {
            wxLogTrace( traceLibraries, "WARNING: missing global table (%s)", magic_enum::enum_name( aType ) );
            return std::nullopt;
        }
                                                       ...
                                                       bool aIncludeInvalid ) const
{
    ...
    switch( aScope )
    {
    case LIBRARY_TABLE_SCOPE::GLOBAL: tables = { std::views::all( m_tables ) }; break;

    case LIBRARY_TABLE_SCOPE::PROJECT: tables = { std::views::all( m_projectTables ) }; break;

    case LIBRARY_TABLE_SCOPE::BOTH: tables = { std::views::all( m_tables ), std::views::all( m_projectTables ) }; break;

    case LIBRARY_TABLE_SCOPE::UNINITIALIZED: wxFAIL;
    }
    ...
    for( const auto& tableSet : tables )
    {
        for( const std::unique_ptr<LIBRARY_TABLE>& table : tableSet | std::views::values )
...
std::optional<LIBRARY_TABLE_ROW*> LIBRARY_MANAGER::FindRowByURI( LIBRARY_TABLE_TYPE aType, const wxString& aUri,
...
std::optional<LIB_STATUS> LIBRARY_MANAGER::LoadLibraryEntry( LIBRARY_TABLE_TYPE aType, const wxString& aNickname )
{
    if( std::optional<LIBRARY_MANAGER_ADAPTER*> adapter = Adapter( aType ); adapter )
...
void LIBRARY_MANAGER_ADAPTER::ProjectTablesReloaded( std::initializer_list<LIBRARY_TABLE_TYPE> aChangedTables )
{
    ...
    std::erase_if( m_libraries,
                   ...
                   {
                       return !m_manager.GetRow( Type(), aEntry.first, LIBRARY_TABLE_SCOPE::PROJECT ).has_value();
                   } );
}
...
std::vector<LIBRARY_TABLE_ROW*> LIBRARY_MANAGER_ADAPTER::Rows( LIBRARY_TABLE_SCOPE aScope, bool aIncludeInvalid ) const
{
    ...
    for( const LIBRARY_TABLE_ROW* row : m_manager.Rows( Type() ) )
    {
        ...
        else
        {
            ...
            ret.emplace_back( std::make_pair(
                    row->Nickname(),
                    LIB_STATUS( { .load_status = LOAD_STATUS::LOAD_ERROR,
...
wxString LIBRARY_MANAGER_ADAPTER::GetLibraryLoadErrors() const
{
    ...
    for( const auto& [nickname, status] : GetLibraryStatuses() )
    {
        if( status.load_status == LOAD_STATUS::LOAD_ERROR && status.error )
        {
            ...
            errors += wxString::Format( _( "Library '%s': %s" ), nickname, status.error->message );
        }
    }
...
{
    auto reloadScope =
            [&]( LIBRARY_TABLE_SCOPE aScopeToReload, std::map<wxString, LIB_DATA>& aTarget, std::shared_mutex& aMutex )
    {
        ...
        if( wasLoaded )
        {
            ...
            if( !result.has_value() )
            {
                wxLogTrace( traceLibraries, "ReloadLibraryEntry: failed to reload %s (%s): %s", aNickname,
                            magic_enum::enum_name( aScopeToReload ), result.error().message );
            }
        }
    };
...
bool LIBRARY_MANAGER_ADAPTER::CreateLibrary( const wxString& aNickname )
{
    if( LIBRARY_RESULT<LIB_DATA*> result = loadIfNeeded( aNickname ); result.has_value() )
    {
        ...
        {
            wxLogTrace( traceLibraries, "CreateLibrary: IO_ERROR for %s: %s", aNickname, ioe.What() );
            return false;
        }
        catch( const std::exception& e )
        {
            wxLogTrace( traceLibraries, "CreateLibrary: std::exception for %s: %s", aNickname, e.what() );
            return false;
        }
    }
...
LIBRARY_RESULT<LIB_DATA*> LIBRARY_MANAGER_ADAPTER::loadFromScope( const wxString& aNickname, LIBRARY_TABLE_SCOPE aScope,
                                                                  ...
                                                                  std::shared_mutex&            aMutex )
{
    ...
    if( !present )
    {
        if( auto result = m_manager.GetRow( Type(), aNickname, aScope ) )
        {
            ...
            wxLogTrace( traceLibraries, "Library %s (%s) not yet loaded, will attempt...", aNickname,
                        magic_enum::enum_name( aScope ) );
...
LIBRARY_RESULT<LIB_DATA*> LIBRARY_MANAGER_ADAPTER::loadIfNeeded( const wxString& aNickname )
{
    LIBRARY_RESULT<LIB_DATA*> result =
            loadFromScope( aNickname, LIBRARY_TABLE_SCOPE::PROJECT, m_libraries, m_librariesMutex );
...
void LIBRARY_MANAGER_ADAPTER::AsyncLoad()
{
    ...
    std::erase_if( m_futures,
                   ...
                   {
                       return aFuture.valid() && aFuture.wait_for( 0s ) == std::future_status::ready;
                   } );
    ...
    auto check = [&]( const wxString& aLib, std::map<wxString, LIB_DATA>& aMap, std::shared_mutex& aMutex )
    {
        std::shared_lock lock( aMutex );
                ...
                [this, workQueue, workIndex]()
                {
                    while( true )
                    {
                        ...
                        if( result.has_value() )
                        {
                            ...
                            {
                                {
                                    std::unique_lock lock( work.scope == LIBRARY_TABLE_SCOPE::GLOBAL
                                                                   ? globalLibsMutex()
                                                                   : m_librariesMutex );
                            ...
                            {
                                std::unique_lock lock( work.scope == LIBRARY_TABLE_SCOPE::GLOBAL ? globalLibsMutex()
                                                                                                 : m_librariesMutex );
                                lib->status.load_status = LOAD_STATUS::LOAD_ERROR;
                                lib->status.error = LIBRARY_ERROR( { e.What() } );
                                wxLogTrace( traceLibraries, "%s: plugin threw exception: %s", work.nickname, e.What() );
                            }
                        }
                        else
                        {
                            ...
                            std::map<wxString, LIB_DATA>& target =
                                    ( work.scope == LIBRARY_TABLE_SCOPE::GLOBAL ) ? globalLibs() : m_libraries;

                            target[work.nickname].status =
                                    LIB_STATUS( { .load_status = LOAD_STATUS::LOAD_ERROR, .error = result.error() } );
                        }
