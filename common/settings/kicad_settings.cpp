...
const nlohmann::json PCM_DEFAULT_REPOSITORIES = nlohmann::json::array( { nlohmann::json( {
        ...
        m_ShowAssistantPanel( false )
{
    ...
    m_params.emplace_back( new PARAM<int>( "appearance.assistant_panel_width", &m_AssistantPanelWidth, 380 ) );
    m_params.emplace_back( new PARAM<bool>( "aui.show_history_panel", &m_ShowHistoryPanel, false ) );
    m_params.emplace_back( new PARAM<bool>( "aui.show_assistant_panel", &m_ShowAssistantPanel, false ) );
    ...
    m_params.emplace_back( new PARAM_LAMBDA<nlohmann::json>(
            ...
            [&]() -> nlohmann::json
            {
                ...
                {
                    js.push_back(
                            nlohmann::json( { { "name", pair.first.ToUTF8() }, { "url", pair.second.ToUTF8() } } ) );
                }
            ...
            [&]( const nlohmann::json aObj )
            {
                ...
                for( const auto& entry : aObj )
                {
                    ...
                    m_PcmRepositories.emplace_back( std::make_pair( wxString( entry["name"].get<std::string>() ),
                                                                    wxString( entry["url"].get<std::string>() ) ) );
                }
            },
            PCM_DEFAULT_REPOSITORIES ) );

    m_params.emplace_back( new PARAM<wxString>( "pcm.last_download_dir", &m_PcmLastDownloadDir, "" ) );
