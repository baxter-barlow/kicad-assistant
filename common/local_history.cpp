...
void LOCAL_HISTORY::UnregisterSaver( const void* aSaverObject )
{
    ...
    {
        m_savers.erase( it );
        wxLogTrace( traceAutoSave, wxS( "[history] Unregistered saver %p (total=%zu)" ), aSaverObject,
                    m_savers.size() );
    }
}
...
bool LOCAL_HISTORY::RunRegisteredSaversAndCommit( const wxString& aProjectPath, const wxString& aTitle )
{
    ...
    for( const auto& [saverObject, saver] : m_savers )
    {
        ...
        wxLogTrace( traceAutoSave, wxS( "[history] saver %p produced %zu entries (total=%zu)" ), saverObject,
                    fileData.size() - before, fileData.size() );
    }
    ...
    m_pendingFuture = GetKiCadThreadPool().submit_task(
            [this, projectPath = aProjectPath, title = aTitle, data = std::move( fileData )]() mutable -> bool
            {
...
{
    wxLogTrace( traceAutoSave, wxS( "[history] background: writing %zu entries for '%s'" ), aFileData.size(),
                aProjectPath );
    ...
    for( const HISTORY_FILE_DATA& entry : aFileData )
    {
        if( !entry.content.empty() )
        {
            ...
            if( fp.IsOpened() )
            {
                ...
                fp.Close();
                wxLogTrace( traceAutoSave, wxS( "[history] background: wrote %zu bytes to '%s'" ), buf.size(),
                            entry.path );
            }
            else
            {
                wxLogTrace( traceAutoSave, wxS( "[history] background: failed to open '%s' for writing" ), entry.path );
            }
        }
        else if( !entry.sourcePath.IsEmpty() )
        {
            wxCopyFile( entry.sourcePath, entry.path, true );
            wxLogTrace( traceAutoSave, wxS( "[history] background: copied '%s' -> '%s'" ), entry.sourcePath,
                        entry.path );
        }
    }
    ...
    int rc = git_commit_create( &commit_id, repo, "HEAD", sig.get(), sig.get(), nullptr, msg.mb_str().data(),
                                indexTree.get(), parents, parents ? &constParent : nullptr );
                                                      ...
                                                      const std::vector<wxString>& aFiles, const wxString& aTitle )
{
    ...
    std::unique_ptr<git_commit, decltype( &git_commit_free )> parent( rawParent, &git_commit_free );
    ...
    for( size_t i = 0; i < numChangedFiles; ++i )
    {
        ...
        std::unique_ptr<git_patch, decltype( &git_patch_free )> patch( rawPatch, &git_patch_free );
        size_t                                                  context = 0, adds = 0, dels = 0;
        git_patch_line_stats( &context, &adds, &dels, patch.get() );
        size_t updated = std::min( adds, dels );
        adds -= updated;
        dels -= updated;
        msg << wxS( "\n" ) << wxString::FromUTF8( delta->new_file.path ) << wxS( " " ) << adds << wxS( "/" ) << dels
            << wxS( "/" ) << updated;
    }
...
bool LOCAL_HISTORY::CommitSnapshot( const std::vector<wxString>& aFiles, const wxString& aTitle )
{
    ...
    if( !lock.IsLocked() )
    {
        wxLogTrace( traceAutoSave, wxS( "[history] CommitSnapshot failed to acquire lock: %s" ), lock.GetLockError() );
        return false;
    }
...
bool LOCAL_HISTORY::HeadNewerThanLastSave( const wxString& aProjectPath )
{
    ...
    for( size_t i = 0; i < tags.count; ++i )
    {
        ...
        if( git_reference_lookup( &ref, repo,
                                  ( wxS( "refs/tags/" ) + wxString::FromUTF8( tags.strings[i] ) ).mb_str().data() )
            == 0 )
        {
                                               ...
                                               const wxString& aMessage )
{
    ...
    if( !lock.IsLocked() )
    {
        wxLogTrace( traceAutoSave, wxS( "[history] CommitDuplicateOfLastSave: Failed to acquire lock for %s" ),
                    aProjectPath );
        return false;
    }
    ...
    int      rc = git_commit_create( &newCommitOid, repo, "HEAD", sig.get(), sig.get(), nullptr, msg.mb_str().data(),
                                     lastTree, parents, parents ? parentArray : nullptr );
    if( headCommit )
...
bool LOCAL_HISTORY::EnforceSizeLimit( const wxString& aProjectPath, size_t aMaxBytes, PROGRESS_REPORTER* aReporter )
{
    ...
    for( size_t idx = 0; idx < keep.size(); ++idx )
    {
        ...
        git_signature_new( &sigAuthor, origAuthor->name, origAuthor->email, origAuthor->when.time,
                           origAuthor->when.offset );
        git_signature_new( &sigCommitter, origCommitter->name, origCommitter->email, origCommitter->when.time,
                           origCommitter->when.offset );
    ...
    for( const auto& tt : tagTargets )
    {
        ...
        {
            git_oid tag_oid;
            git_tag_create_lightweight( &tag_oid, newRepo, tt.first.mb_str().data(), obj, 0 );
            git_object_free( obj );
        }
    }
 ...
 */
void collectFilesInDirectory( const wxString& aRootPath, const wxString& aSearchPath, std::set<wxString>& aFiles )
{
                        ...
                        std::vector<wxString>& aFilesToDelete )
{
            ...
            [&]( const wxString& dirPath, const wxString& relativeBase )
    {
        ...
        {
            // Skip special directories
            if( filename == wxS( ".history" ) || filename == wxS( ".git" ) || filename == wxS( "_restore_backup" )
                || filename == wxS( "_restore_temp" ) )
            {
                cont = dir.GetNext( &filename );
                continue;
            }
            ...
            wxFileName fullPath( dirPath, filename );
            wxString   relativePath = relativeBase.IsEmpty() ? filename : relativeBase + wxS( "/" ) + filename;
 ...
 */
bool confirmFileDeletion( wxWindow* aParent, const std::vector<wxString>& aFilesToDelete, bool& aKeepAllFiles )
{
    ...
    dlg.SetExtendedMessage( _( "Choosing 'Keep All Files' will restore the selected commit but retain any existing "
 ...
 */
bool backupCurrentFiles( const wxString& aProjectPath, const wxString& aBackupPath, const wxString& aTempRestorePath,
                         bool aKeepAllFiles, std::set<wxString>& aBackedUpFiles )
{
    ...
    while( cont )
    {
            ...
            && filename != wxS( "_restore_temp" ) )
        {
            ...
            if( shouldBackup )
            {
                ...
                {
                    wxLogTrace( traceAutoSave, wxS( "[history] backupCurrentFiles: Creating backup directory %s" ),
                                aBackupPath );
                    wxFileName::Mkdir( aBackupPath, 0777, wxPATH_MKDIR_FULL );
                }

                wxLogTrace( traceAutoSave, wxS( "[history] backupCurrentFiles: Backing up '%s' to '%s'" ),
                            source.GetFullPath(), dest.GetFullPath() );

                if( !wxRenameFile( source.GetFullPath(), dest.GetFullPath() ) )
                {
                    wxLogTrace( traceAutoSave, wxS( "[history] backupCurrentFiles: Failed to backup '%s'" ),
                                source.GetFullPath() );
                    return false;
                }
                           ...
                           std::set<wxString>& aRestoredFiles )
{
    ...
    while( cont )
    {
        ...
        wxLogTrace( traceAutoSave, wxS( "[history] restoreFilesFromTemp: Restoring '%s' to '%s'" ),
                    source.GetFullPath(), dest.GetFullPath() );

        if( !wxRenameFile( source.GetFullPath(), dest.GetFullPath() ) )
        {
            wxLogTrace( traceAutoSave, wxS( "[history] restoreFilesFromTemp: Failed to move '%s'" ),
                        source.GetFullPath() );
            return false;
        }
 ...
 */
void rollbackRestore( const wxString& aProjectPath, const wxString& aBackupPath, const wxString& aTempRestorePath,
                      const std::set<wxString>& aBackedUpFiles, const std::set<wxString>& aRestoredFiles )
{
    ...
    for( const wxString& filename : aRestoredFiles )
    {
        ...
        wxLogTrace( traceAutoSave, wxS( "[history] rollbackRestore: Removing '%s'" ), toRemove.GetFullPath() );
    ...
    if( wxDirExists( aBackupPath ) )
    {
        for( const wxString& filename : aBackedUpFiles )
        {
            ...
            if( source.Exists() )
            {
                ...
                wxLogTrace( traceAutoSave, wxS( "[history] rollbackRestore: Restored '%s'" ), dest.GetFullPath() );
            }
        }
    }
 ...
 */
bool recordRestoreInHistory( git_repository* aRepo, git_commit* aCommit, git_tree* aTree, const wxString& aHash )
{
    ...
    git_oid           new_id;
    const git_commit* constParent = parent;
    int               result = git_commit_create( &new_id, aRepo, "HEAD", sig, sig, nullptr, msg.mb_str().data(), aTree,
                                                  parent ? 1 : 0, parent ? &constParent : nullptr );
...
bool LOCAL_HISTORY::RestoreCommit( const wxString& aProjectPath, const wxString& aHash, wxWindow* aParent )
{
    ...
    {
        wxLogTrace( traceAutoSave, wxS( "[history] RestoreCommit: Failed to create temp directory %s" ),
                    tempRestorePath );
        git_tree_free( tree );
        git_commit_free( commit );
        return false;
    ...
    if( !backupCurrentFiles( aProjectPath, backupPath, tempRestorePath, keepAllFiles, backedUpFiles ) )
    {
        rollbackRestore( aProjectPath, backupPath, tempRestorePath, backedUpFiles, restoredFilesSet );
        git_tree_free( tree );
        git_commit_free( commit );
        return false;
    }
