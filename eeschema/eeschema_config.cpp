...
void SCH_EDIT_FRAME::LoadDrawingSheet()
{
    ...
    wxString filename = resolver.ResolvePath( settings.m_SchDrawingSheetFileName, Prj().GetProjectPath(),
...
void SCH_EDIT_FRAME::ShowSchematicSetupDialog( const wxString& aInitialPage )
{
    ...
    NULLER raii_nuller( (void*&) m_schematicSetupDialog );
...
void SCH_EDIT_FRAME::saveProjectSettings()
{
    ...
    if( !BASE_SCREEN::m_DrawingSheetFileName.IsEmpty() )
    {
        ...
        wxFileName layoutfn( resolve.ResolvePath( BASE_SCREEN::m_DrawingSheetFileName, Prj().GetProjectPath(),
                                                  { Schematic().GetEmbeddedFiles() } ) );
    ...
    {
        Prj().GetProjectFile().m_IP2581Bom.schRevision = Schematic().RootScreen()->GetTitleBlock().GetRevision();
    }
...
void SCH_EDIT_FRAME::LoadSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    m_show_assistant_panel = cfg->m_AuiPanels.show_assistant_panel;
...
void SCH_EDIT_FRAME::SaveSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    if( cfg )
    {
        ...
        if( m_assistantPanel )
        {
            ...
            cfg->m_AuiPanels.show_assistant_panel = m_show_assistant_panel;
...
void SCH_BASE_FRAME::LoadSettings( APP_SETTINGS_BASE* aCfg )
{
    ...
    {
        aCfg->m_Window.grid.grids.emplace_back(
                GRID{ "User Grid", aCfg->m_Window.grid.user_grid_x, aCfg->m_Window.grid.user_grid_y } );
