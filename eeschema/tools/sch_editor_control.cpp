...
BOX2I expandedSelectionBox( const SCH_SELECTION& aSelection )
{
    ...
    if( bbox.GetWidth() > 0 && bbox.GetHeight() > 0 )
        bbox.Inflate( bbox.GetWidth() * clipboardBboxInflation, bbox.GetHeight() * clipboardBboxInflation );
...
wxImage renderSelectionToImageForClipboard( SCH_EDIT_FRAME* aFrame, const SCH_SELECTION& aSelection, const BOX2I& aBBox,
...
int SCH_EDITOR_CONTROL::Revert( const TOOL_EVENT& aEvent )
{
    ...
    if( !IsOK( m_frame, msg ) )
    {
        ...
        if( wasOnSubsheet )
        {
            m_frame->GetToolManager()->RunAction<SCH_SHEET_PATH*>( SCH_ACTIONS::changeSheet, &originalSheet );
            wxSafeYield();
        }
...
int SCH_EDITOR_CONTROL::SimProbe( const TOOL_EVENT& aEvent )
{
    ...
    picker->SetMotionHandler(
            [this]( const VECTOR2D& aPos )
            {
                ...
                collector.Collect( m_frame->GetScreen(), { SCH_ITEM_LOCATE_WIRE_T, SCH_PIN_T, SCH_SHEET_PIN_T }, aPos );
        ...
        if( ( tmp.m_Path.empty() && aClipPath.empty() ) || ( !aClipPath.empty() && tmp.m_Path.EndsWith( aClipPath ) ) )
        {
            ...
            wxLogTrace( traceSchPaste,
                        wxS( "Pasting found symbol instance with reference %s, unit %d:\n"
                             "\tClipboard path: %s\n"
                             "\tSymbol UUID: %s." ),
                        tmp.m_Reference, tmp.m_Unit, aClipPath.AsString(), aSymbol->m_Uuid.AsString() );
...
int SCH_EDITOR_CONTROL::Paste( const TOOL_EVENT& aEvent )
{
    ...
    for( EDA_ITEM* item : loadedItems )
    {
        ...
        if( item->Type() == SCH_SYMBOL_T )
        {
            ...
            LIB_SYMBOL* clipLibSymbol = ( clipIt != tempScreen->GetLibSymbols().end() ) ? clipIt->second : nullptr;
        ...
        else if( item->Type() == SCH_SHEET_T )
        {
            ...
            for( SCH_SHEET_PATH& sheetPath : sheetPathsForScreen )
            {
                SCH_SHEET_PATH subPath =
                        updatePastedSheet( sheet, sheetPath, clipPath, ( forceKeepAnnotations && annotateAutomatic ),
    ...
    if( !annotatedSymbols.empty() )
    {
        ...
        for( SCH_SHEET_PATH& path : sheetPathsForScreen )
        {
            ...
            {
                annotatedSymbols[path].ReannotateByOptions( annotateOrder, annotateAlgo, annotateStartNum, existingRefs,
                                                            false, &hierarchy );
            }
            ...
            for( const SCH_SHEET_PATH& pastedSheetPath : pastedSheets[path] )
            {
                ...
                {
                    annotatedSymbols[pastedSheetPath].ReannotateByOptions(
                            annotateOrder, annotateAlgo, annotateStartNum, existingRefs, false, &hierarchy );
                }
    ...
    if( !selection.Empty() )
    {
        if( aEvent.IsAction( &ACTIONS::duplicate ) )
        {
            ...
            auto processPt = [&]( const VECTOR2I& pt )
            {
            ...
            if( closest_dist == INT_MAX )
            {
                for( EDA_ITEM* item : selection.Items() )
                {
                    ...
                    case SCH_SHAPE_T:
                    {
                        ...
                        case SHAPE_T::CIRCLE: processPt( shape->GetCenter() ); break;
                    ...
                    default: processPt( item->GetPosition() ); break;
                    }
                }
            }
...
int SCH_EDITOR_CONTROL::ShowAssistantPanel( const TOOL_EVENT& aEvent )
{
    getEditFrame<SCH_EDIT_FRAME>()->ToggleAssistantPanel();
    return 0;
}
...
void SCH_EDITOR_CONTROL::setTransitions()
{
    ...
    Go( &SCH_EDITOR_CONTROL::ShowAssistantPanel, SCH_ACTIONS::showAssistantPanel.MakeEvent() );
